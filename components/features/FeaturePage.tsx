...
export function FeaturePage({ slug }: { slug: FeatureSlug }) {
  ...
  return (
    <main>
      ...
      <section className="py-14 sm:py-18 lg:py-20"><Container><div className="max-w-2xl"><Eyebrow>Benefits</Eyebrow><h2 className="mt-3 text-3xl font-extrabold tracking-tight text-ink sm:text-4xl">Placeholder benefits for this pillar</h2></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{feature.benefits.map(([Icon, title, description]) => <article key={title} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand/10 text-brand"><Icon className="h-6 w-6" /></div><h3 className="mt-5 text-xl font-extrabold text-ink">{title}</h3><p className="mt-3 leading-7 text-slate-700">{description}</p></article>)}</div></Container></section>
      <SectionBand><Container className="grid items-center gap-8 lg:grid-cols-[0.85fr_1fr]"><FeatureMock slug={slug} /><div><Eyebrow>Visual mock</Eyebrow><h2 className="mt-3 text-3xl font-extrabold tracking-tight text-ink sm:text-4xl">A clear dashboard for {feature.eyebrow.toLowerCase()}.</h2><p className="mt-5 text-lg leading-8 text-slate-700">Placeholder copy for the relevant visual mock. Final product screenshots and approved copy can replace this scaffold without changing the layout.</p></div></Container></SectionBand>
      ...
      <SectionBand className="bg-brand text-white"><Container className="text-center"><h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Ready to make retirement feel safer?</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-8 text-white/85">Placeholder CTA band copy for {feature.eyebrow}. Start with the free Safety Score or choose the pillar you want to explore next.</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Button href={feature.href} variant="secondary">{feature.cta}</Button><Button href="/features" variant="ghost" className="text-white hover:bg-white/10 hover:text-white">Compare pillars</Button></div></Container></SectionBand>
    </main>
  );
}
