...
export type CuratedAlert = {
  title: string;
  body: string;
  category: "benefit" | "inflation" | "market" | "scam" | "tax" | "medicare" | "ss";
  states: string[];
  min_age: number | null;
  action_line: string;
...
function isAlert(v: unknown): v is CuratedAlert {
  ...
  return !!a && typeof a.title === "string" && typeof a.body === "string" &&
    ["benefit", "inflation", "market", "scam", "tax", "medicare", "ss"].includes(a.category) &&
...
export async function curateAlertsFromSource(source: string): Promise<CuratedAlert[]> {
  try {
    ...
    const message = await client.messages.create({
      ...
      max_tokens: 1800,
      temperature: 0,
      system: `${SAFETY_SYSTEM}\n\nYou curate RetireGuard alert drafts for human review. Never fabricate. Use only facts explicitly present in the source. If unsure, leave states empty and min_age null. All generated rows are drafts and must be human-approved before publishing. Major Congress, IRS, tax-law, interest-rate, inflation, Social Security/COLA, Medicare, and market-moving items are allowed only when stated in SOURCE.`,
      messages: [{ role: "user", content: `Convert the raw SOURCE into one or more content_items drafts. Return ONLY strict JSON array with fields: title, body, category (benefit|inflation|market|scam|tax|medicare|ss), states (US state abbreviations array; [] for national/unknown), min_age (number or null), action_line, source_url (or null), published_at (or null), expires_at (or null), status (always "draft"). Rewrite and summarize only facts present in SOURCE. Never invent scams, dates, figures, agencies, or eligibility.\n\nSOURCE:\n${source.slice(0, 12000)}` }],
